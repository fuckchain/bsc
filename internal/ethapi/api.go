...
// The sender is responsible for signing the transactions and using the correct
// nonce and ensuring validity
func (s *BundleAPI) CallBundlePro(ctx context.Context, args CallBundleProArgs) (map[string]interface{}, error) {
	if len(args.Txs) == 0 {
		return nil, errors.New("bundle missing txs")
	}
	if len(args.BlockNumbers) != len(args.Txs) {
		return nil, errors.New("bundle invalid blockNumbers")
	}
	if len(args.Timestamps) != len(args.Txs) {
		return nil, errors.New("bundle invalid timestamps")
	}
	...
	for i, tx := range txs {
		...
		blockNumber := big.NewInt(int64(args.BlockNumbers[i].Int64()))
